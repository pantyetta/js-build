...
「なんとなく使っている」を「仕組みが分かっている」に変えるための読み物です。
これと、**実際に動かして出力を比べる 4 つのサンプル**で教材が出来ています。

| 学ぶこと | 仕組みの解説 | 手を動かす |
|---|---|---|
| この教材が前提にしている JS の常識と歴史（モジュール・ESM・npm・トランスパイル） | [js-background.md](./js-background.md) | — |
| バンドラなし。素のブラウザに何が出来ないかを体感する | [00-no-bundler.md](./00-no-bundler.md) | [`00-no-bundler/`](../00-no-bundler/README.md) |
| バンドラの原理を、一番薄いレイヤーで見る | [01-esbuild.md](./01-esbuild.md) | [`01-esbuild/`](../01-esbuild/README.md) |
| 今の標準。dev と build でアーキテクチャが違う理由 | [02-vite.md](./02-vite.md) | [`02-vite/`](../02-vite/README.md) |
| `__webpack_require__` の正体と、loader / plugin の世界観 | [03-webpack.md](./03-webpack.md) | [`03-webpack/`](../03-webpack/README.md) |

- **前提知識に不安があれば** [js-background.md](./js-background.md) を先に（ES2015・モジュールの歴史・npm）
- **この文書**が地図（全体像）。まずこれを通しで読む
- **`docs/0X-*.md`** が、そのツールが内部で何をやっているかの解説
- **`0X-*/README.md`** が、どのコマンドを叩き DevTools のどこを見て何を壊すかの実験ノート
- 読む順番は 00 → 03。前の章の内容を次の章が使う
...
（用語や年表まで含めたフル版は [js-background.md](./js-background.md)）
...
│                  CSS: lightningcss / postcss                   │  （新しい構文を対象ブラウザ向けに落とす）
├───────────────────────────────────────────────────────────────┤
...
**lightningcss も同じ**で、「CSS のトランスパイラ＋バンドラ＋minifier」を1つでこなす（Parcel 由来）。
Vite 8 が CSS 処理を esbuild から lightningcss に替えたとき、変わったのは minify だけでなく
**構文変換（`color-scheme: light dark` の lowering など）も**。単なる minifier の差し替えではない。
...
[`02b-vite7/`](../02b-vite7/README.md) に Vite 7 で固定した同じアプリを置いてあります。
`02-vite`（Vite 8）と両方ビルドして、依存・minifier の癖・出力を実際に見比べられます。
「dev は 7 も 8 も同じ、変わるのは本番バンドラだけ」がそこで確認できます。
...
この文書が地図、`docs/0X-*.md` が仕組みの解説、各フォルダの README が実地です。

```sh
# 0. まず素のブラウザの限界を知る（インストール不要）
cd 00-no-bundler && npx serve .

# 1. バンドラの原理（設定が全部 JS で書いてあるので読みやすい）
cd 01-esbuild && npm install
                 npm run build      # dist/ を覗く
                 npm run dev        # watch + サーバー（HMR は無い）

# 2. 今の標準
cd 02-vite && npm install
              npm run dev           # DevTools の Network タブを開いたまま起動する
              npm run build
              npm run preview

# 3. loader / plugin の世界
cd 03-webpack && npm install
                 npm run dev
                 npm run build
                 npm run build:dev  # production との出力差を見る
```

`01`〜`03` は初回だけ `npm install` が要ります（`node_modules/` は git 管理外）。

各ツールの仕組みの解説:
[00 バンドラなし](./00-no-bundler.md) ·
[01 esbuild](./01-esbuild.md) ·
[02 Vite](./02-vite.md) ·
[03 webpack](./03-webpack.md)

各フォルダの実験ノート:
[00](../00-no-bundler/README.md) ·
[01](../01-esbuild/README.md) ·
[02](../02-vite/README.md) ·
[03](../03-webpack/README.md)
...
- **Vite 公式「なぜ Vite なのか」** — https://ja.vite.dev/guide/why （日本語。この文書の 5 章と同じ話を公式が説明している）
